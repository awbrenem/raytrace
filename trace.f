...
c modified o.s. 2006
c       magnetic field scaling added  (MAGNETIC-FIELD multiplier, bmult)
c modified o.s. 2008 05 14 (ver. 080514)
c       1000 km replaced by reference altitude in the input and output files
c       altitude of the base of the ionosphere added to input and output files
c       (important only for ionospheric ray tracing - 0+ whistlers)
c       version information added
c modified o.s. 2008 07 31 (ver. 080731)
c     mag field model returned back to dipole coordinates
c        using only the G(1,0) term (updated to the IGRF2005 dipole moment)
c      bug introduced 2006, -> G(1,0), G(1,1) and H(1,1) of IGRF~1962 used -
c        corresp to geogr. coordinates)
c
c modified o.s. 2008 09 11 (ver. 080911)
c      IGRF model added using GEOPACK (for Santolik et al. 2009) -
c        modified input file, IGRF -> output in geogr. coordinates
c
c When modified update the version strings at places marked by
c  VERSION INFORMATION HERE  >>>>>
c
c---->
c----> REFERENCES:
c---->
...
c  fr�quence de la r�sonance hybride basse, in Plasma Waves in Space
...
c
c Santolik, O., F. Nemec, M. Parrot, D. Lagoutte, L. Madrias,
c  J.J. Berthelier, Analysis methods for multi-component wave measurements
c  on board the DEMETER spacecraft,
c  Planetary and Space Science 54, 512-527, 2006.
c
c Santolik, O., J. Chum, M. Parrot, D. A. Gurnett, J. S. Pickett,
c  and N. Cornilleau-Wehrlin, Propagation of whistler mode chorus
c  to low altitudes: Spacecraft observations of structured ELF hiss,
c  J. Geophys. Res., 111, A10208, doi:10.1029/2005JA011462, 2006.
c
c Santolik, O., M. Parrot, U. S. Inan, D. Buresova, D. A. Gurnett
c and J. Chum, Propagation of unducted whistlers from their
c source lightning: a case study,
c J. Geophys. Res., 114, A03212, doi:10.1029/2008JA013776., 2009.
c
c---->
c----> INPUT FILE:
c---->
...
c___________________________________________________:_______|______|
cGEOMAG MODEL-COORDINATES (0:DIPOLE-MAG, 1:IGRF-GEO):   0
c     IGRF OR DIPOLE INTERPOLATION DATE (IYEAR,IDOY):   2005    209
c                          MAGNETIC-FIELD multiplier:   1.00000
...
c              ALTITUDE of the IONOSPHERIC BASE (km):   89.
c                   ALTITUDE of REFERENCE LEVEL (km):   22195.0
c             DENSITY AT the REFERENCE LEVEL (cm^-3):   120.000
c                      nH+/ne at the REFERENCE LEVEL:   1.00000
c                     nHe+/ne at the REFERENCE LEVEL:        0.
c                      n0+/ne at the REFERENCE LEVEL:        0.
c                                    temperature (K):   1000.00
...
cReg Path(Re)  TimeG(s) %dTimeG/T %dPath/WL    WL(km)  R(Re) Alt(km) Lat^o Long^o      L     F/Fge     F/FgH   n(cm-3)   %H+  %He+   %O+ LHR/kHz RefrInd VeK^o AzK^o ThK^o ThG^o PhK^o Gen^o Res^o
...
c            270 - toward W
c Gen^o   = Aprox. Gendrin's angle for the whistler mode (ThK for ThG=0) in degrees.
...
c        radio waves, Cambridge University Press, New York, 1988, isbn:0-521-36952-2:
...
c----added, os 09/2008
      COMMON /GEOPACK2/ GGP(105),HGP(105),RECGP(105)
       integer IYEAR,IDOY imagmod,i,j,k
c----added, os 09/2008
       real bmult,HBASE, HZERO
      write(*,*) '--------------------------'
c  VERSION INFORMATION HERE  >>>>>
      write(*,*) 'Start ray tracing v.080911'
      write(*,*) '--------------------------'
        krun=0
c       input file
      open(1,action='read',file='trace_in.txt')
c----added, os 09/2008
      read(1,'(//52x,I14)') imagmod
       read(1,'(52x,2I7)') IYEAR,IDOY
       call RECALC_08 (IYEAR,IDOY,0,0,0,0,0,0)
c      SUBROUTINE RECALC_08 (IYEAR,IDAY,IHOUR,MIN,ISEC,VGSEX,VGSEY,VGSEZ)

c----added, os 09/2008
      read(1,'(52x,g14.6)') bmult
       read(1,'(52x,I14)') KDUCT
c      read(1,'(52x,g14.6)') KDUCT
       read(1,'(52x,g14.6)')EP
...
c      write(*,*) PHI0
      ...
      IECR=4
        LUT=3
        open(LUT,ACTION='write',file='trace_ta.txt')
c _____Output file header
      WRITE(LUT,'(a,g14.6,a)')
c  VERSION INFORMATION HERE  >>>>>
     . 'TRACE (ver. 080911) @ frequency of ',f,' Hz'
      write(LUT,606)
606     FORMAT('Reg Path(Re)  TimeG(s) %dTimeG/T %dPath/WL    WL(km)',
     &'    R(Re) Alt(km)    Lat^o   Long^o     L(Re)',
     &'     F/Fge     F/FgH   n(cm-3)   %H+  %He+   %O+ LHR/kHz',
     &'   RefrInd VeK^o AzK^o ThK^o ThG^o PhK^o Gen^o Res^o')
c _____
c
      IF(imagmod.EQ.1) then
c  ----- IGRF -geographic coordinates
        do 9 i=1,11
         do 9 j=1,i
          k=i*(i-1)/2+j
c     ------------- IGRF debugging printouts - compare with old (1962) data block
c          write(*,'(a,2i2,a,f9.6,a,2i2,a,f9.6)')
c     ,     'old G(',i,j,')=',G(i,j),
c     ,     '     old H(',i,j,')=',H(i,j)
c          write(*,'(a,i2,a,f9.6,a,i2,a,f9.6)')
c     ,     'new GGP(',k,')=',-GGP(k)/1e5,
c     ,     '     new HGP(',k,')=',-HGP(k)/1e5
c    c      write(*,*)
c     -------------------------------
          G(i,j)=-GGP(k)/1e9*bmult
          H(i,j)=-HGP(k)/1e9*bmult
9       continue
        write(*,'(a,I5,a,I3)')
     ,  '---- GEOGRAPHIC COORDINATES USED. IGRF MODEL',IYEAR,'/',IDOY
        write(*,*) '----'
      else
c  ----- dipole from IGRF -mag coordinates
        DO 7 I=1,11
        DO 7 J=1,11
          G(I,J)=0.
          H(I,J)=0.
...
c		write(*,*) HGP(2)
c		write(*,*) bmult
c		write(*,*) GGP(2)*GGP(2)+GGP(3)*GGP(3)+HGP(3)*HGP(3)
c		write(*,*) sqrt(GGP(2)*GGP(2)+GGP(3)*GGP(3)+HGP(3)*HGP(3))
c		write(*,*) sqrt(GGP(2)*GGP(2)+GGP(3)*GGP(3)+HGP(3)*HGP(3))/(1d9)

		
        G(2,1)=bmult*sqrt( GGP(2)*GGP(2)+GGP(3)*GGP(3)+HGP(3)*HGP(3))
        G(2,1)=G(2,1)/1d9
c        G(2,1)=bmult*sqrt( GGP(2)*GGP(2)+GGP(3)*GGP(3)+HGP(3)*HGP(3))/(bmult*1d9)
c        write(*,*) G(2,1)
		write(*,*) '---dipmodel vals---'
        write(*,'(a,I5,a,I3,a,f7.0)')
     ,   '---- DIPOLE MAG. COORDINATES USED. DIPOLE MODEL',
     ,          IYEAR,'/',IDOY,'   G(2,1)=',G(2,1)*1e9
        write(*,*) '----'
      endif
c  ----------------------------------------
      ...
      write(LUT,'(a,a,a)') '______________________________',
c----added, os 09/2008
     ,             '_____________________:_______|______|'
      write(LUT,'(a52,I14)')
     . 'GEOMAG MODEL-COORDINATES (0:DIPOLE-MAG, 1:IGRF-GEO):',imagmod
      write(LUT,'(a52,2I7)')
...
c----added, os 09/2008
     ...
     . '  DUCT L-CENTRAL POSITION:',AL0
      write(LUT,'(a52,g14.6)')
     . 'ALTITUDE of the IONOSPHERIC BASE (km):',HZERO
      write(LUT,'(a52,g14.6)')
     . 'ALTITUDE of REFERENCE LEVEL (km):',HBASE
      write(LUT,'(a52,g14.6)')
     . 'DENSITY AT the REFERENCE LEVEL (cm^-3):',AN0
      write(LUT,'(a52,g14.6)')
     . ' nH+/ne at the REFERENCE LEVEL:',ALP0(1)
      write(LUT,'(a52,g14.6)')
     . ' nHe+/ne at the REFERENCE LEVEL:',ALP0(2)
      write(LUT,'(a52,g14.6)')
     . ' n0+/ne at the REFERENCE LEVEL:',ALP0(3)
      write(LUT,'(a52,g14.6)')
     . ' temperature (K):',THERM
      write(LUT,'(a52,g14.6)')
     ...
     ,               ALT1,THET0,PHI0
      ...
      DATA RT,THERM,RBASE,RZERO,SCBOT /6371.2,1000.,43317.,6460.,140./
...
605   FORMAT(I2,5G10.3,F9.5,F8.0,2F9.4,G10.3,2f10.3,F10.2,3F6.1,
     &F8.3,G10.3,7F6.1)
